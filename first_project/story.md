# project_story
- 총 인원 : 7명
- 주제 : 금융 데이터 분석(신용카드 사용자의 신용등급 예측, 카드 고객의 이탈여부)
...
## 8일차, Credit Final
- 화상 회의
>1. 내가 설계한 모델링이 성능이 가장 좋아서 나의 모델 및 데이터를 기준으로 파생적으로 각자 모델기법 분석
>>1. 팀원1 : PCA, K-means 기법을 사용해서 모델링 성능 테스트
>>2. 팀원2 : 직업 컬럼 삭제, 나이 도수분표 컬럼 생성 후 모델링 성능 테스트
>>3. 팀원3 : 내가 만든 모델의 Parameters 변환 및 다른 모델 기법 테스트
>>4. 팀원4 : 점진선택법을 바탕으로 feature 중요도를 바탕으로 columnm 추가 및 삭제 후 모델링 성능 테스
- 진행 결과
>1. 중복값 제거를 통한 성능 테스트 결과 성능과 Log_loss의 값은 좋았으나, Target 0의 데이터 수가 부족해 0의 Recall값이 현저히 떨어짐
>2. 해당 문제를 해결하기 위해 PCA 및 SMOTE_overSampling기법을 통해 0의 데이터를 늘리고 차원을 축소해본 결과 0의 Recall값이 높아졌으나 다수의 데이터를 차지하는 2에 대한 Recall값이 떨어졌기 때문에 기존의 사용한 모델 및 EDA 분석을 통한 데이터셋 사용
>3. 최종 결과 : Log_loss값은 0.61754으로 나왔고,  accuracy는 0.765935으로 나왔다.
## 9일차, NewData_이탈 고객 예측
- 화상 회의
>1. 기존의 Credit_grade 예측 프로젝트를 마치고, 이번에는 고객의 이탈을 예측하는 머신러닝 모델을 구축하는 프로젝트를 진행했다.
>2. 해당 데이터는 Kaggle을 통해 가져왔으며, 약 20개의 컬럼을 바탕으로 이탈하는지, 안 하는지는 예측하는 이진 분류 라벨값을 예측한다.
>3. 가장 큰 문제는 이탈 고객의 데이터가 약 16%로 전체 데이터 중 적은 비율을 보이기 때문에 해당 문제를 해결해야하는 것이다.
>4. 진행 방식은 성능 테스트는 5개의 DecisionTree, RandomForest, xgboost. lgbmboost 4개의 모델을 통해 성능을 테스트할 예정이며, 함수를 만들고 팀원간의 함수를 통해 성능을 비교할 예쩡이다.
>5. 그리고 매일 회의에서 가장 유의미한 성과를 낸 EDA데이터셋을 공유하고, 해당 데이터를 다시 EDA하는 반복 과정을 통해 모델의 성능을 높일 계획이다.
- 진행 결과
>1. 일단 이상치 데이터, 결측값 등을 각 Feature별로 시각화를 통해 확인 결과 결측값은 존재하지 않으며 이상치 데이터는 분석 작업 이후 처리할 예정이다.
>2. 각 변수의 분석 결과 Target구분하는 유의미한 Feature은 크게 보이지 않으며, 일단 모델의 성능을 테스트할 함수부터 만들기로 했다.
>3. 4개의 모델로 분석 결과 Decision	0.937808 / Randomforest	0.948174 / XGBC	0.968411 / LGBMC	0.968411
>4. 다만, 0 라벨에 대한 Recall값이 0.86으로 낮기 때문에 해당 부분을 개선하고자 시도
## 10일차, Recall값 개선 시도
- 화상 회의
